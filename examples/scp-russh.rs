...
use remotefs_ssh::{NoCheckServerKey, RusshSession, ScpFs, SshOpts};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut scp_fs: ScpFs<RusshSession<NoCheckServerKey>> = ScpFs::russh(
    ...
    // list files
    let files = scp_fs.list_dir(Path::new("/tmp"))?;
    for file in files {
