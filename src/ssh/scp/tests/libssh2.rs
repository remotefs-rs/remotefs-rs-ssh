use std::io::{Cursor, ErrorKind, Seek as _};
...
type Client = ScpFs<crate::ssh::backend::LibSsh2Session>;
...
fn should_initialize_scp_filesystem() {
    let client = ScpFs::libssh2(SshOpts::new("127.0.0.1"));
...
fn should_be_sync_and_send() {
    ...
    let client = ScpFs::libssh2(SshOpts::new("scp"));
    is_sync(&client);
    is_send(client);
}
